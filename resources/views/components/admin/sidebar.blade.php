...
>
    ...
    <nav class="flex-1 space-y-1 px-3">
        ...
        <a href="{{ route('admin.dashboard') }}" class="{{ request()->routeIs('admin.dashboard') ? $activeClass : $inactiveClass }} {{ $navItemClass }}">
            <svg class="size-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z"></path></svg>
            Overview
        </a>

        <a href="{{ route('admin.vendors.index') }}" class="{{ request()->routeIs('admin.vendors.*') ? $activeClass : $inactiveClass }} {{ $navItemClass }}">
        ...
        @endphp
        <a href="{{ route('admin.packages.index') }}" class="{{ request()->routeIs('admin.packages.*') ? $activeClass : $inactiveClass }} {{ $navItemClass }} justify-between">
            ...
            @if($pendingPackages > 0)
                <span class="bg-[#ff3b30] text-white text-[10px] font-bold h-5 min-w-[20px] px-1.5 flex items-center justify-center rounded-full shadow-sm shrink-0">
                    {{ $pendingPackages > 10 ? '10+' : $pendingPackages }}
                </span>
            @endif
        </a>
        ...
        @endphp
        <a href="{{ route('admin.payments.index') }}" class="{{ request()->routeIs('admin.payments.*') ? $activeClass : $inactiveClass }} {{ $navItemClass }} justify-between">
            <div class="flex items-center gap-3">
                <svg class="size-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            ...
            @if($pendingPayments > 0)
                <span class="bg-[#ff3b30] text-white text-[10px] font-bold h-5 min-w-[20px] px-1.5 flex items-center justify-center rounded-full shadow-sm shrink-0">
                    {{ $pendingPayments > 10 ? '10+' : $pendingPayments }}
                </span>
            @endif
        </a>
        ...
        @endphp
        <a href="{{ route('admin.promos.index') }}" class="{{ request()->routeIs('admin.promos.*') ? $activeClass : $inactiveClass }} {{ $navItemClass }} justify-between">
            <div class="flex items-center gap-3">
            ...
            @if($totalPendingPromos > 0)
                <span class="bg-[#ff3b30] text-white text-[10px] font-bold h-5 min-w-[20px] px-1.5 flex items-center justify-center rounded-full shadow-sm shrink-0">
                    {{ $totalPendingPromos > 10 ? '10+' : $totalPendingPromos }}
                </span>
            @endif
        </a>

        <a href="{{ route('admin.withdrawals.index') }}" class="{{ request()->routeIs('admin.withdrawals.*') ? $activeClass : $inactiveClass }} {{ $navItemClass }}">
        ...
        <a href="{{ route('admin.reports.monthly') }}" class="{{ request()->routeIs('admin.reports.*') ? $activeClass : $inactiveClass }} {{ $navItemClass }}">
