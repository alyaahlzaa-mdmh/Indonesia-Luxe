<x-layouts::auth>
    ...
    <div
        ...
        <div class="text-center mb-6"><img src="{{ asset('images/logo.png') }}" alt="Indonesia Luxe" class="h-16 mx-auto mb-3">
            <h1 class="text-2xl text-white mb-1">{{ __('auth.register') }}</h1>
            <p class="text-white/70 text-sm">{{ __('auth.register_desc') }}</p>
        </div>
        ...
        <form method="POST" action="{{ route('register.store') }}" class="space-y-4" @submit="loading = true">
            @csrf
            <input type="hidden" name="role" :value="role === 'traveler' ? 'customer' : 'vendor'">

            <div>
                <label class="text-sm text-white/80 block mb-1">{{ __('auth.name') }}</label>
                <div class="relative">
                    ...
                    <input type="text" name="name" value="{{ old('name') }}" required placeholder="{{ __('auth.name_placeholder') }}" class="w-full bg-white/15 border rounded-xl pl-10 pr-4 py-3 outline-none focus:ring-2 focus:ring-amber-400 text-white placeholder-white/40 @error('name') border-red-500 @else border-white/30 @enderror">
                </div>
            </div>

            <div x-show="role === 'vendor'">
                <label class="text-sm text-white/80 block mb-1">{{ __('auth.business_name') }}</label>
                <div class="relative">
                    ...
                    <input type="text" name="business_name" value="{{ old('business_name') }}" :required="role === 'vendor'" placeholder="{{ __('auth.business_name_placeholder') }}" class="w-full bg-white/15 border rounded-xl pl-10 pr-4 py-3 outline-none focus:ring-2 focus:ring-amber-400 text-white placeholder-white/40 @error('business_name') border-red-500 @else border-white/30 @enderror">
                </div>
            </div>
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="email" name="email" value="{{ old('email') }}" required placeholder="email@example.com" class="w-full bg-white/15 border rounded-xl pl-10 pr-4 py-3 outline-none focus:ring-2 focus:ring-amber-400 text-white placeholder-white/40 @error('email') border-red-500 @else border-white/30 @enderror">
                </div>
            </div>

            <div>
                <label class="text-sm text-white/80 block mb-1">{{ __('auth.phone') }}</label>
                <div class="relative">
                    ...
                    <input type="tel" name="phone" value="{{ old('phone') }}" required placeholder="08xxxxxxxxxx" class="w-full bg-white/15 border rounded-xl pl-10 pr-4 py-3 outline-none focus:ring-2 focus:ring-amber-400 text-white placeholder-white/40 @error('phone') border-red-500 @else border-white/30 @enderror">
                </div>
            </div>

            <div>
                <label class="text-sm text-white/80 block mb-1">{{ __('auth.password') }}</label>
                <div class="relative">
                    ...
                    <input :type="showPassword ? 'text' : 'password'" name="password" required placeholder="{{ __('auth.password_placeholder') }}" class="w-full bg-white/15 border rounded-xl pl-10 pr-12 py-3 outline-none focus:ring-2 focus:ring-amber-400 text-white placeholder-white/40 @error('password') border-red-500 @else border-white/30 @enderror">
            ...
            <div>
                <label class="text-sm text-white/80 block mb-1">{{ __('auth.confirm_password') }}</label>
                <div class="relative">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-lock absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-white/50">
                        <rect width="18" height="11" x="3" y="11" rx="2" ry="2"></rect>
                        <path d="M7 11V7a5 5 0 0 1 10 0v4"></path>
                    </svg>
                    <input :type="showPassword ? 'text' : 'password'" name="password_confirmation" required placeholder="{{ __('auth.confirm_password_placeholder') }}" class="w-full bg-white/15 border border-white/30 rounded-xl pl-10 pr-4 py-3 outline-none focus:ring-2 focus:ring-amber-400 text-white placeholder-white/40">
                    <button type="button" @click="showPassword = !showPassword" class="absolute right-3 top-1/2 -translate-y-1/2 text-white/50 hover:text-white/80 transition-colors">
                        <svg x-show="!showPassword" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-eye w-5 h-5">
                            <path d="M2.062 12.348a1 1 0 0 1 0-.696 10.75 10.75 0 0 1 19.876 0 1 1 0 0 1 0 .696 10.75 10.75 0 0 1-19.876 0"></path>
                            <circle cx="12" cy="12" r="3"></circle>
            ...
            <button type="submit" :disabled="loading" class="w-full bg-amber-500 hover:bg-amber-600 disabled:bg-amber-500/50 disabled:cursor-not-allowed text-white py-3 rounded-xl flex items-center justify-center gap-2 transition border border-amber-400 mt-6 min-h-[50px]">
                <template x-if="!loading">
                    <div class="flex items-center gap-2">
                        ...
                        <span x-text="role === 'traveler' ? '{{ __('auth.register') }}' : '{{ __('auth.register_vendor') }}'"></span>
                    </div>
                </template>
                <template x-if="loading">
        ...
        <div class="mt-6 text-center">
            <p class="text-white/70 text-sm">{{ __('auth.already_have_account') }} <a class="text-amber-400 hover:text-amber-300" href="/login" data-discover="true">{{ __('auth.login') }}</a></p>
        </div>
    </div>
</x-layouts::auth>