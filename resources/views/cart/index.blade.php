<x-layouts.site :title="__('cart.title')">
    <!-- <h1 class="text-2xl font-semibold mb-4">Shopping Cart</h1> -->
    <a href="{{ route('home') }}" class="flex items-center gap-1.5 text-gray-500 hover:text-amber-600 transition mb-4 text-sm font-semibold">
    ...
    <div class="space-y-3" x-data="{
        selectedIds: {{ $cart->items->pluck('id')->map(fn($id) => (string)$id)->toJson() }},
        items: {{ $cart->items->map(fn($item) => ['id' => (string)$item->id, 'total' => (float)$item->line_total])->toJson() }},
        get subtotal() {
            return this.items
        ...
        <h1 class="text-2xl text-gray-900 mb-6">
            ...
            </svg>{{ __('cart.shopping_cart') }} ({{ $cart->items->count() }} {{ __('cart.item') }})
        </h1>
        ...
        <div class="grid lg:grid-cols-3 gap-8">
            <div class="lg:col-span-2 space-y-4">
                <div class="flex items-center justify-between">
                    <button @click="toggleAll()" class="flex items-center gap-2 text-sm text-gray-600 hover:text-amber-600 transition">
                        ...
                        <span x-text="selectedIds.length === items.length ? '{{ __('cart.deselect_all') }}' : '{{ __('cart.select_all') }}'"></span>
                    </button>
                ...
                <div class="bg-white rounded-2xl shadow overflow-hidden transition"
                    :class="selectedIds.includes('{{ $item->id }}') ? 'ring-2 ring-amber-400 shadow-md' : 'ring-1 ring-gray-200 hover:ring-2 hover:ring-amber-400'">
                    <div class="p-4 flex gap-4">
                        <label class="shrink-0 mt-1 self-start cursor-pointer">
                            <input type="checkbox" value="{{ $item->id }}" x-model="selectedIds" class="hidden">
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex items-start justify-between">
                                <div class="min-w-0 flex-1">
                                    <span class="text-[10px] font-bold uppercase tracking-wider bg-amber-100 text-amber-700 px-2 py-0.5 rounded-full">{{ $item->tourPackage->category->name }}</span>
                                    <h3 class="text-gray-900 mt-1 font-semibold line-clamp-1">{{ $item->tourPackage->title }}</h3>
                                    <p class="text-xs text-gray-500">{{ __('cart.by') }} {{ $item->tourPackage->vendor->name }}</p>
                                </div>
                            ...
                            <div class="flex flex-wrap items-center gap-3 mt-2">
                                <div class="flex items-center gap-1 bg-amber-50 text-amber-700 px-2.5 py-1 rounded-full text-xs">
                                    ...
                                    <span>{{ $item->slot->departure_date->format('d M Y') }}</span>
                                </div>
                                <div class="flex items-center border rounded-lg bg-white overflow-hidden">
                                    <form method="POST" action="{{ route('cart.items.update', $item) }}">
                                        ...
                                        <button type="submit" class="px-2 py-1 hover:bg-amber-50 text-gray-600 transition disabled:opacity-30" {{ $item->quantity <= 1 ? 'disabled' : '' }}>-</button>
                                    </form>
                            ...
                            <div class="mt-2 text-right">
                                <span class="text-gray-400 text-xs mr-2">{{ $item->quantity }} x Rp {{ number_format($item->price_per_person, 0, ',', '.') }}</span>
                                <span class="text-amber-600 font-bold">Rp {{ number_format($item->line_total, 0, ',', '.') }}</span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div>
                <div class="bg-white rounded-2xl p-6 shadow-lg shadow-gray-100 sticky top-20 border border-gray-100">
                    <h2 class="text-lg font-bold text-gray-900 mb-4">{{ __('cart.order_summary') }}</h2>
                    <div class="space-y-3 text-sm border-b pb-4 mb-4 max-h-[300px] overflow-y-auto">
                        @foreach ($cart->items as $item)
                        <div class="flex justify-between text-gray-600 gap-4" x-show="selectedIds.includes('{{ $item->id }}')">
                            <span class="line-clamp-1 flex-1">{{ $item->tourPackage->title }}</span>
                            <span class="shrink-0 font-medium whitespace-nowrap">Rp {{ number_format($item->line_total, 0, ',', '.') }}</span>
                        </div>
                        @endforeach
                    </div>
                    <div class="pt-2">
                        <div class="flex justify-between text-lg text-gray-900">
                            <span class="font-medium">{{ __('cart.total') }}<span class="text-xs text-gray-400 block font-normal"><span x-text="selectedCount"></span> {{ __('cart.selected_items') }}</span></span>
                    ...
                    <a class="block text-center text-gray-500 hover:text-amber-600 text-sm mt-4 transition" href="{{ route('tours.index') }}">
        ...
        <div class="flex flex-col items-center justify-center py-24 bg-white rounded-3xl shadow-sm border border-gray-100">
            ...
            <h2 class="text-2xl font-bold text-gray-800 mb-2">{{ __('cart.cart_empty') }}</h2>
            <p class="text-gray-500 mb-8 max-w-xs text-center">{{ __('cart.cart_empty_desc') }}</p>
            <a href="{{ route('tours.index') }}" class="inline-flex items-center gap-2 bg-amber-500 hover:bg-amber-600 text-white px-8 py-3.5 rounded-full transition font-bold shadow-lg shadow-amber-200">
                {{ __('cart.search_tour_now') }}
