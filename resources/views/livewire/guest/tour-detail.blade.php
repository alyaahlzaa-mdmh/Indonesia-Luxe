<div class="py-4 font-sans bg-gray-50 min-h-screen">
    ...
    <div class="relative w-full h-[300px] md:h-[400px] rounded-2xl overflow-hidden mb-8 shadow-sm group"
        ...
        <template x-for="(img, index) in images" :key="index">
            <div class="absolute inset-0 w-full h-full transition-opacity duration-1000 ease-in-out"
                ...
                <img :src="img" alt="{{ $tourPackage->title }}" class="w-full h-full object-cover transition-transform duration-[5000ms] ease-linear" :class="activeImg === index ? 'scale-105' : 'scale-100'" />
            </div>
        </template>
        ...
        <div class="absolute top-6 left-6 z-20">
            <a href="{{ route('tours.index') }}" class="bg-white/90 backdrop-blur-sm text-slate-800 p-2 rounded-full inline-flex items-center justify-center hover:bg-white transition-colors cursor-pointer shadow-sm">
        ...
        <div class="absolute top-6 right-6 z-20">
            ...
            <button wire:click.prevent="toggleWishlist({{ $tourPackage->id }})"
                class="bg-white/90 backdrop-blur-sm p-2 rounded-full inline-flex items-center justify-center hover:bg-white transition-all cursor-pointer shadow-sm {{ $isFavorited ? 'text-[#FF7A45]' : 'text-slate-500' }}">
                <x-guest.icons.heart class="w-5 h-5" :fill="$isFavorited ? 'currentColor' : 'none'" />
            </button>
        </div>
        ...
        <div class="absolute bottom-6 left-6 right-6 flex flex-col items-start text-white z-20">
            <span class="bg-amber-500 text-xs font-bold tracking-wider uppercase px-3 py-1 rounded-md mb-3 shadow-sm">
                {{ $tourPackage->category->name }} • {{ $typeLabel }}
            </span>
            <h1 class="text-3xl md:text-5xl font-bold mb-2">{{ $tourPackage->title }}</h1>
            <p class="text-white/80 flex items-center gap-2">
                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" />
                </svg>
                {{ __('tour_detail.meeting_point') }}: {{ $tourPackage->meeting_point ?? '-' }}
            </p>
        </div>
    ...
    <div class="grid gap-8 lg:grid-cols-3">
        ...
        <div class="lg:col-span-2 space-y-8">
            ...
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-wrap gap-6 items-center justify-between md:justify-start md:gap-12">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <p class="text-[11px] text-gray-500 uppercase tracking-widest font-semibold">{{ __('tour_detail.duration') }}</p>
                        <p class="font-bold text-slate-800">{{ $tourPackage->duration_hours ? __('tour_detail.duration_hours', ['hours' => $tourPackage->duration_hours]) : __('tour_detail.duration_days') }}</p>
                    </div>
                </div>
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <p class="text-[11px] text-gray-500 uppercase tracking-widest font-semibold">{{ __('tour_detail.max_pax') }}</p>
                        <p class="font-bold text-slate-800">{{ __('tour_detail.pax_unit', ['count' => $tourPackage->max_participants ?? '-']) }}</p>
                    </div>
                </div>
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <p class="text-[11px] text-gray-500 uppercase tracking-widest font-semibold">{{ __('tour_detail.type') }}</p>
                        <p class="font-bold text-slate-800">{{ $typeLabel }}</p>
                    </div>
                </div>
                <div class="flex items-center gap-3">
                    <div class="flex items-center gap-1 text-amber-500 bg-amber-50 p-2.5 rounded-lg border border-amber-100">
                        ...
                        <span class="font-extrabold text-lg">{{ number_format((float) ($tourPackage->reviews_avg_rating ?? 0), 1) }}</span>
                    </div>
                    <div>
                        <a href="#reviews" class="text-sm font-semibold text-slate-800 hover:text-amber-500 underline">{{ __('tour_detail.reviews_count', ['count' => $tourPackage->reviews_count]) }}</a>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-gray-100">
                <h2 class="text-xl md:text-2xl font-bold mb-4 text-slate-800">{{ __('tour_detail.about_activity') }}</h2>
                <div class="prose prose-sm md:prose-base text-gray-600 max-w-none leading-relaxed">
                    <p>{{ $tourPackage->description }}</p>
                </div>

                <h3 class="text-lg font-bold mt-8 mb-4 text-slate-800 border-b pb-2">{{ __('tour_detail.highlights') }}</h3>
                <ul class="space-y-3">
                    @foreach([__('tour_detail.highlight_1'), __('tour_detail.highlight_2'), __('tour_detail.highlight_3'), __('tour_detail.highlight_4')] as $highlight)
                ...
                <h3 class="text-lg font-bold mt-8 mb-4 text-slate-800 border-b pb-2">{{ __('tour_detail.whats_included') }}</h3>
                <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                    @foreach([__('tour_detail.inclusion_guide'), __('tour_detail.inclusion_ticket'), __('tour_detail.inclusion_transport'), __('tour_detail.inclusion_lunch'), __('tour_detail.inclusion_documentation'), __('tour_detail.inclusion_firstaid')] as $inc)
                    <div class="flex items-center gap-2 bg-gray-50 p-2 rounded-lg border border-gray-100">
                        <svg class="w-4 h-4 text-emerald-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                        </svg>
                        <span class="text-sm font-medium text-slate-700">{{ $inc }}</span>
                    </div>
                    @endforeach
                </div>
                ...
                <div class="space-y-8 pl-4 border-l-2 border-amber-500/30 ml-2 md:ml-4">
                    @foreach($grouped as $day => $dayItineraries)
                    <div class="relative">
                        <!-- Day Dot -->
                        ...
                        <h4 class="font-bold text-amber-600 mb-5 text-[17px] leading-none">{{ __('tour_detail.day', ['number' => $day]) }}</h4>

                        <div class="space-y-5">
                            @foreach($dayItineraries as $item)
                            <div class="relative pl-5">
                                ...
                                <div class="bg-gray-50 border border-gray-100 p-3.5 md:p-4 rounded-xl shadow-[0_2px_8px_rgba(0,0,0,0.02)] group hover:border-amber-200 hover:bg-amber-50/30 transition-colors">
                                    <div class="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-3 mb-1.5">
                                        @if($item->time)
                                        <span class="bg-amber-100 text-amber-700 text-[11px] font-bold px-2 py-0.5 rounded-md w-fit">{{ $item->time }}</span>
                                        @endif
                                        <h5 class="font-serif font-bold text-slate-800 text-[15px] leading-snug">{{ $item->title }}</h5>
                                    </div>
                                    @if($item->description)
                                    <p class="text-[13px] text-slate-600 leading-relaxed">{{ $item->description }}</p>
                                    @endif
                                </div>
                            </div>
                            @endforeach
            ...
            <div id="reviews" class="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-gray-100">
                <div class="flex justify-between items-center border-b pb-4 mb-6">
                    <h2 class="text-xl md:text-2xl font-bold text-slate-800">{{ __('tour_detail.user_reviews') }}</h2>
                    <div class="flex items-center gap-1 bg-amber-100 text-amber-600 px-3 py-1 rounded-full text-sm font-bold">
                        ★ {{ number_format((float) ($tourPackage->reviews_avg_rating ?? 0), 1) }}
                    </div>
                </div>
                ...
                <div class="space-y-6">
                    ...
                    <div class="border-b border-gray-50 pb-6 last:border-0 last:pb-0">
                        <div class="flex justify-between items-start mb-3">
                            <div class="flex gap-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-[11px] text-gray-500">{{ $review->created_at->format('d M Y') }}</p>
                                </div>
                            </div>
                        ...
                        <h4 class="font-bold text-slate-800 text-sm mb-1">{{ $review->title ?? __('tour_detail.default_review_title') }}</h4>
                        <p class="text-sm text-gray-600 bg-gray-50 p-3 rounded-lg border border-gray-100 italic">"{{ $review->comment }}"</p>
                    </div>
                    @empty
                    <div class="text-center py-8">
                        ...
                        <p class="text-sm text-gray-500">{{ __('tour_detail.no_reviews') }}</p>
                    </div>
                    @endforelse
                </div>
            </div>
        ...
        <div class="relative lg:h-auto"
            x-data="{
                isExpanded: false,
                startY: 0,
                selectedSlotId: '{{ count($tourPackage->slots) > 0 ? $tourPackage->slots[0]->id : '' }}',
                slots: {{ count($tourPackage->slots) > 0 ? json_encode($tourPackage->slots->map(function($s) { return ['id' => $s->id, 'date' => $s->departure_date->format('d M Y'), 'price' => $s->price_per_person, 'quota' => $s->availableQuota()]; })) : '[]' }},
                quantity: 1,
                selectedPickup: '',
                ...
                get basePrice() { return this.currentSlot && this.currentSlot.price ? this.currentSlot.price : {{ $tourPackage->price_per_person }}; },
                get total() { return this.basePrice * this.quantity; },
                get quotaLabel() {
                    if (!this.currentSlot) return '';
                    const q = this.currentSlot.quota;
                    if (q <= 3) return '{{ __('tour_detail.almost_full') }}';
                    if (q <= 10) return '{{ __('tour_detail.available') }}';
                    return '{{ __('tour_detail.available') }}';
                },
                get quotaColor() {
                    if (!this.currentSlot) return '';
                    const q = this.currentSlot.quota;
            ...
            <div
                ...
                <div
                    ...
                    <div class="w-full flex justify-between items-center transition-opacity duration-200" :class="isExpanded ? 'opacity-0 pointer-events-none h-0' : 'opacity-100'">
                        <div class="flex flex-col">
                            <span class="text-[10px] text-gray-400 uppercase tracking-widest font-semibold leading-none mb-1">{{ __('tour_detail.price_per_person') }}</span>
                ...
                <div class="px-5 pb-5 lg:p-0 overflow-y-auto overflow-x-hidden max-h-[80vh] lg:max-h-none custom-scrollbar overscroll-contain"
                    ...
                    @if(count($tourPackage->slots) > 0)

                    <!-- Price Header -->
                    <div class="p-6 border-b border-gray-100">
                        <p class="text-[11px] uppercase tracking-wider font-semibold text-gray-400 mb-2">{{ __('tour_detail.price_per_person') }}</p>
                        <div class="flex items-baseline gap-x-1.5">
                            <span class="text-3xl font-bold text-[#b48c47]" x-text="formatRp(basePrice)"></span>
                            <span class="text-sm text-gray-400">{{ __('tour_detail.per_person') }}</span>
                        </div>
                    </div>

                    <form id="add-to-cart-form" action="{{ route('cart.items.store') }}" method="POST" class="p-6 space-y-6">
                        @csrf
                        ...
                        <div class="space-y-3">
                            ...
                            <div class="relative pl-6">
                                <select name="tour_departure_slot_id" x-model="selectedSlotId" @change="quantity = 1" class="w-full bg-transparent border-none p-0 text-[15px] font-bold text-gray-700 focus:ring-0 cursor-pointer appearance-none">
                                    @foreach ($tourPackage->slots as $slot)
                                    <option value="{{ $slot->id }}">
                                        {{ $slot->departure_date->format('j M Y') }}
                                    </option>
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <div class="space-y-1">
                            <label class="text-[13px] text-gray-400 font-medium flex items-center gap-2">
                                ...
                                {{ __('tour_detail.remaining_quota') }}
                            </label>
                            <div class="pl-6">
                        ...
                        <div class="space-y-3">
                            <label class="text-[13px] text-gray-400 font-medium flex items-center gap-2">
                                ...
                                {{ __('tour_detail.participant_count') }}
                            </label>
                            ...
                            <p class="text-[11px] text-gray-400">{{ __('tour_detail.max_participants', ['count' => $tourPackage->max_participants ?? '-']) }}</p>
                        </div>
                        ...
                        @endphp
                        @if($isAdminPackage || $tourPackage->pickupPoints->count() > 0)
                        <div class="space-y-3">
                            <label class="text-[13px] text-gray-400 font-medium flex items-center gap-2">
                                <svg class="w-4 h-4 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                ...
                                {{ __('tour_detail.pickup_point') }} <span class="text-red-400">*</span>
                            </label>
                            <div class="relative group">
                                ...
                                <select name="pickup_point" x-model="selectedPickup" required class="w-full pl-10 pr-10 py-3 bg-white border border-blue-200 rounded-xl text-gray-500 font-medium text-sm focus:ring-2 focus:ring-blue-100 focus:border-blue-400 outline-none appearance-none transition-all">
                                    <option value="">{{ __('tour_detail.select_pickup') }}</option>
                        ...
                        <div class="space-y-3">
                            <label class="text-[13px] text-gray-400 font-medium flex items-center gap-2">
                                ...
                                {{ __('tour_detail.promo_code') }}
                            </label>
                            <div class="flex gap-2">
                                <input type="text" placeholder="{{ __('tour_detail.promo_placeholder') }}" class="flex-1 min-w-0 border border-gray-200 rounded-xl px-4 py-3 text-sm focus:border-orange-300 focus:ring-2 focus:ring-orange-50 outline-none transition-all" />
                        ...
                        <div class="bg-gray-50/50 rounded-xl p-5 space-y-3 border border-gray-100">
                            <div class="flex justify-between items-center text-sm text-gray-500">
                                <span x-text="formatRp(basePrice) + ' × ' + quantity + ' {{ __('tour_detail.person_unit') }}'"></span>
                                <span x-text="formatRp(total)"></span>
                            </div>
                            <div class="pt-3 border-t border-gray-100 flex justify-between items-center">
                                <span class="font-bold text-gray-700">{{ __('tour_detail.total') }}</span>
                        ...
                        <div class="space-y-3 pt-2">
                            ...
                            <button type="submit" id="add-to-cart-btn" class="w-full bg-[#ff9e52] hover:bg-[#f98b2f] text-white rounded-xl py-4 flex items-center justify-center gap-3 transition-all font-bold shadow-md shadow-orange-100 group relative overflow-hidden">
                                ...
                                <span>{{ __('tour_detail.add_to_cart') }}</span>
                            </button>
                            <button type="submit" name="redirect_to" value="checkout" class="w-full bg-[#b18a45] hover:bg-[#9d793a] text-white rounded-xl py-4 flex items-center justify-center gap-3 transition-all font-bold shadow-md shadow-amber-100">
                                ...
                                {{ __('tour_detail.book_now') }}
                            </button>
                            @else
                            <button disabled class="w-full bg-gray-200 text-gray-400 rounded-xl py-4 font-bold cursor-not-allowed">
                                {{ __('tour_detail.buyer_only') }}
                            </button>
                            @endif
                            @else
                            <a href="{{ route('login') }}" class="w-full bg-[#b18a45] hover:bg-[#9d793a] text-white rounded-xl py-4 flex items-center justify-center gap-3 transition-all font-bold shadow-md shadow-amber-100">
                                <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 16l-4-4m0 0l4-4m-4 4h14m-5 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h7a3 3 0 013 3v1" />
                                </svg>
                                {{ __('tour_detail.login_to_book') }}
                            </a>
                            @endauth
                        </div>
                    ...
                    <div class="p-8 text-center">
                        <div class="mb-6">
                            <p class="text-[11px] uppercase font-bold text-gray-400 tracking-wider mb-1">{{ __('tour_detail.offer_price') }}</p>
                            <p class="text-3xl font-bold text-[#b48c47]">Rp {{ number_format($tourPackage->price_per_person, 0, ',', '.') }}<span class="text-sm text-gray-400 font-normal"> {{ __('tour_detail.per_person') }}</span></p>
                        </div>
                        <div class="py-10 bg-red-50/50 rounded-2xl border border-red-100">
                            ...
                            <p class="text-lg font-bold text-red-600">{{ __('tour_detail.sold_out') }}</p>
                            <p class="text-sm text-red-400 mt-1">{{ __('tour_detail.no_schedule') }}</p>
                        </div>
                    </div>
                    @endif
    ...
    <!-- Vendor Info -->
    @if($tourPackage->vendor)
    <div class="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-gray-100 mt-8">
        <h2 class="text-xl font-bold text-slate-800 mb-4">{{ __('tour_detail.about_organizer') }}</h2>
        <div class="flex items-start gap-4">
            <div class="w-14 h-14 rounded-full bg-amber-100 text-amber-600 flex items-center justify-center font-bold text-xl flex-shrink-0">
                {{ substr($tourPackage->vendor->vendorProfile->business_name ?? $tourPackage->vendor->name, 0, 1) }}
            </div>
            <div class="flex-1">
                <h3 class="font-bold text-lg text-slate-800">{{ $tourPackage->vendor->vendorProfile->business_name ?? $tourPackage->vendor->name }}</h3>
                @if($tourPackage->vendor->vendorProfile?->address)
                <p class="text-sm text-gray-500 flex items-center gap-1 mt-1">
                    <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" />
                    </svg>
                    {{ $tourPackage->vendor->vendorProfile->address }}
                </p>
                @endif
                @if($tourPackage->vendor->vendorProfile?->business_description)
                <p class="text-sm text-gray-600 mt-3 leading-relaxed">{{ $tourPackage->vendor->vendorProfile->business_description }}</p>
                @endif
            </div>
        </div>
    </div>
