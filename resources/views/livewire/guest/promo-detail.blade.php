<div class="bg-[#FDFBF7] min-h-screen w-full flex flex-col">
    ...
    <div class="relative w-full mb-12">
        ...
        <div class="relative z-20 w-full max-w-5xl mx-auto px-4 pt-10 pb-[100px] text-center">
            ...
            <div class="text-left mb-6">
                <a href="{{ route('home') }}" class="inline-flex items-center justify-center gap-1.5 px-4 py-2 rounded-full bg-white/10 hover:bg-white/20 text-white/90 text-xs font-semibold backdrop-blur-sm transition-colors border border-white/10">
                    <x-guest.icons.chevron-left class="w-3.5 h-3.5" />
                    {{ __('promo.back') }}
                </a>
            </div>
            ...
            <div class="mx-auto flex flex-col items-center">
                ...
                <h1 class="text-4xl md:text-[56px] font-serif text-white mb-6 text-shadow-sm font-medium">{{ __('promo.hero_title') }}</h1>
                <p class="text-white/90 text-sm md:text-[17px] font-light mb-10 max-w-lg">{{ __('promo.hero_subtitle') }}</p>
                ...
                <div class="flex flex-wrap justify-center gap-3 mb-8">
                    ...
                    <div class="flex items-center gap-2 bg-black/20 border border-white/20 px-5 py-2.5 rounded-full backdrop-blur-md text-white/90 text-[13px] md:text-sm shadow-inner">
                        <svg class="w-4 h-4 opacity-70" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="{{ $badge['icon'] }}"/></svg>
    ...
    <div class="max-w-3xl mx-auto px-4 mb-24 flex flex-col gap-3">
        ...
        <div class="bg-{{ $item['active'] ? 'gradient-to-r from-orange-50/50 to-amber-50/50' : 'white' }} border text-center border-{{ $item['active'] ? 'orange-200' : 'gray-100' }} rounded-2xl p-3 md:p-4 flex flex-col md:flex-row md:items-center justify-between shadow-[0_4px_20px_-8px_rgba({{ $item['active'] ? '206,98,35,0.15' : '0,0,0,0.06' }})] gap-4 {{ !$item['active'] ? 'hover:border-orange-200 transition-colors' : '' }}">
            <div class="flex items-center gap-3 md:gap-4 w-full md:w-auto justify-between md:justify-start">
                <span class="{{ $item['active'] ? 'bg-[#f29150] text-white shadow-sm border-orange-400/50' : 'bg-gray-50 text-gray-500 border-gray-100' }} font-bold text-[11px] md:text-xs px-4 py-2 rounded-xl flex-shrink-0 border">{{ $item['date'] }}</span>
                <span class="text-[13px] md:text-sm font-semibold text-slate-700 md:ml-2">{{ $item['title'] }}</span>
            </div>
            <div class="flex items-center justify-between md:justify-end gap-3 w-full md:w-auto {{ !$item['active'] ? 'bg-gray-50/50' : 'bg-white/50' }} md:bg-transparent p-2 md:p-0 rounded-xl">
                <span class="{{ $item['codeStyle'] }} font-mono text-[11px] md:text-xs px-3 py-1.5 rounded-lg border border-dashed">{{ $item['code'] }}</span>
                <button class="text-[#ce6223] p-1.5 rounded-md hover:bg-orange-50 transition-colors cursor-pointer" title="{{ __('promo.calendar_copy_tooltip') }}">
                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z"/></svg>
                </button>
            </div>
        </div>
    ...
    <div class="max-w-5xl mx-auto px-4 mb-20">
        <x-guest.section-title :title="__('promo.vouchers_title')" :subtitle="__('promo.vouchers_subtitle')" />
        ...
        <div class="grid md:grid-cols-3 gap-5">
            ...
            <div class="bg-gradient-to-br {{ $voucher['gradient'] }} rounded-[24px] p-6 text-white shadow-[0_10px_25px_-5px_{{ $voucher['shadow'] }}] relative overflow-hidden group hover:-translate-y-1 transition-transform duration-300">
                ...
                <p class="{{ $voucher['labelColor'] }} text-[11px] font-bold uppercase tracking-wider mb-2">{{ $voucher['label'] }}</p>
                <div class="flex items-baseline gap-1 mb-2">
                    @if($voucher['discountType'] === 'amount')
                        <span class="text-xl font-medium font-serif">Rp</span>
                        <h3 class="text-[34px] leading-none font-bold font-serif tracking-tight drop-shadow-sm">{{ $voucher['discount'] }}</h3>
                    @else
                        <h3 class="text-5xl font-bold font-serif tracking-tight drop-shadow-sm">{{ $voucher['discount'] }}</h3>
                    @endif
                </div>
                <p class="{{ $voucher['minColor'] }} text-[13px] mb-8 font-light">{{ $voucher['minOrder'] }}</p>
                ...
                <div class="bg-white/15 hover:bg-white/25 border border-white/30 rounded-xl p-3 flex items-center justify-between backdrop-blur-md cursor-pointer transition-colors">
                    <span class="font-mono text-sm font-semibold tracking-wider drop-shadow-sm">{{ $voucher['code'] }}</span>
    ...
    <div class="w-full bg-[#1b1919] text-white/90 overflow-hidden mb-20 py-3.5 border-y border-[#3a3031] shadow-xl relative top-0 flex">
        ...
        <div class="animate-marquee whitespace-nowrap flex gap-8 text-[13px] font-medium tracking-wide items-center w-max">
            ...
            @for ($i = 0; $i < 4; $i++)
                ...
                <span class="flex items-center gap-2"><span>{{ $ticker['emoji'] }}</span>{{ $ticker['text'] }}</span>
    ...
    <div class="max-w-5xl mx-auto px-4 mb-16">
        <x-guest.section-title :title="__('promo.tours_title')" :subtitle="__('promo.tours_subtitle')">
            <!-- Filter Chips -->
            <div class="mt-6 mb-8">
                <x-guest.type-filter-chips 
                    :currentType="$type" 
                    :typeLabels="$typeLabels" 
                    :onlyTypes="['open_trip', 'private_tour', 'hiking_camping', 'snorkeling_diving']" 
                    size="md" 
                />
            </div>
        ...
        <div class="grid gap-5 md:grid-cols-3 lg:grid-cols-4">
            @forelse ($tourPackages as $tourPackage)
                <x-guest.tour-card :tourPackage="$tourPackage" :typeLabels="$typeLabels" variant="promo" />
            @empty
                <x-guest.empty-state 
                    :title="__('promo.tours_empty_title')" 
                    :subtitle="__('promo.tours_empty_subtitle')" 
                    variant="promo" 
                    :showReset="false"
                    :showHome="false"
                />
        ...
        <div class="mt-16 text-center mb-24">
            <a href="{{ route('tours.index') }}" class="inline-flex items-center gap-2 bg-[#FF7A45] hover:bg-[#eb6a35] text-white px-8 md:px-10 py-3.5 rounded-full font-bold text-sm shadow-md shadow-orange-500/20 transition-transform hover:-translate-y-0.5">
                {{ __('promo.tours_book_now') }}
        ...
        <div class="mb-20">
            ...
            <div class="w-full relative rounded-2xl overflow-hidden bg-[#8C4E2D] py-8 mb-8 flex flex-col items-center justify-center shadow-md">
                ...
                <div class="relative z-10 text-center">
                    <p class="text-white/80 text-[10px] uppercase tracking-[0.2em] mb-1 font-medium">{{ __('promo.explore_subtitle') }}</p>
                    <h2 class="text-3xl md:text-[32px] font-serif text-white font-medium">{{ __('promo.explore_title') }}</h2>
                </div>
            </div>
            ...
            <div class="flex flex-col gap-3 mb-8">
                ...
                <div class="flex overflow-x-auto gap-2.5 pb-2 hide-scrollbar">
                    @foreach($locationChips as $i => $chip)
                    <button class="px-6 py-2 rounded-full text-[13px] font-medium transition-colors border shadow-sm flex-shrink-0 {{ $i === 0 ? 'bg-[#FF7A45] text-white border-[#FF7A45]' : 'bg-white text-slate-600 border-gray-200 hover:bg-gray-50' }}">
                        {{ $chip }}
                    </button>
                    @endforeach
                </div>
                <div class="flex overflow-x-auto gap-2 pb-2 hide-scrollbar">
                    @foreach($activityChips as $chip)
            ...
            <!-- Featured Card Grid -->
            <div class="grid gap-5 md:grid-cols-3 lg:grid-cols-4">
                @foreach ($featuredPackages as $tourPackage)
                    <x-guest.tour-card :tourPackage="$tourPackage" :typeLabels="$typeLabels" variant="promo" />
                @endforeach
            </div>
        </div>
        ...
        <div class="mb-10">
            ...
            <div class="w-full relative rounded-2xl overflow-hidden bg-[#603B26] py-8 mb-8 flex flex-col items-center justify-center shadow-md">
                ...
                <div class="relative z-10 text-center">
                    <p class="text-white/80 text-[10px] font-medium mb-1 tracking-wider uppercase">{{ __('promo.planner_subtitle') }}</p>
                    <h2 class="text-2xl md:text-[28px] font-serif text-white font-medium tracking-wide">{{ __('promo.planner_title') }}</h2>
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
                @foreach($plannerFeatures as $feature)
                <a href="#" class="bg-white rounded-[24px] p-6 text-center border border-gray-100/80 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_8px_30px_{{ $feature['hoverShadow'] }}] transition-all duration-300 hover:-translate-y-1 group">
                    <div class="w-[50px] h-[50px] mx-auto {{ $feature['bgColor'] }} rounded-full flex items-center justify-center mb-4 group-hover:scale-110 transition-transform duration-300">
                        <svg class="w-[22px] h-[22px] {{ $feature['textColor'] }}" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                            <path d="{{ $feature['icon'] }}" />
                        </svg>
                    </div>
                    <span class="text-[13px] font-semibold text-slate-700">{{ $feature['title'] }}</span>
                </a>
                @endforeach
            </div>
        </div>
            ...
            <div class="bg-amber-50/50 border border-amber-100/60 rounded-[20px] p-6 md:p-8">
                ...
                <ul class="space-y-3 mb-6">
                    ...
                    <li class="flex items-start gap-3">
                        ...
                        <p class="text-[14px] text-slate-600">{{ $term }}</p>
                    </li>
                    @endforeach
                </ul>
    ...
    <div class="w-full bg-gradient-to-r from-[#b96231] via-[#854121] to-[#3f2b31] py-16 px-4 text-center relative overflow-hidden mt-auto">
        ...
        <div class="relative z-10 max-w-2xl mx-auto flex flex-col items-center">
            ...
            <h2 class="text-2xl md:text-3xl font-serif text-white font-medium mb-3">{{ __('promo.footer_title') }}</h2>
            <p class="text-white/80 text-[14px] mb-8">{{ __('promo.footer_subtitle') }}</p>
            
            <div class="flex flex-col sm:flex-row items-center gap-3">
                <a href="{{ route('tours.index') }}" class="bg-[#f08535] hover:bg-[#e67e32] text-white px-6 py-3 rounded-full text-[14px] font-semibold shadow-lg shadow-orange-500/20 transition-all hover:shadow-orange-500/40 hover:-translate-y-0.5 w-full sm:w-auto flex items-center justify-center gap-2">
                    {{ __('promo.footer_btn_search') }}
                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7" />
                    </svg>
                </a>
