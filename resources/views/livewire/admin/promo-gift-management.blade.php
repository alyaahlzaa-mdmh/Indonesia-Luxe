<div class="space-y-6">
    ...
    <div class="flex w-full max-w-2xl gap-1 rounded-2xl border border-gray-100 bg-white p-1.5 shadow-sm">
        <button wire:click="setType('promo')" class="flex flex-1 items-center justify-center gap-2 rounded-xl py-3 text-sm font-bold transition-all {{ $activeType === 'promo' ? 'border border-[#ffedd5] bg-[#fff8ed] text-[#f59e0b]' : 'text-gray-400 hover:bg-gray-50' }}">
        ...
        <button wire:click="setType('gift_card')" class="flex flex-1 items-center justify-center gap-2 rounded-xl py-3 text-sm font-bold transition-all {{ $activeType === 'gift_card' ? 'border border-[#ede9fe] bg-[#f5f3ff] text-[#7c3aed]' : 'text-gray-400 hover:bg-gray-50' }}">
    ...
    @if (! $isCreating)
        <div class="flex flex-col justify-between gap-5 rounded-3xl border p-5 shadow-sm md:flex-row md:items-center {{ $activeType === 'promo' ? 'border-[#fef3c7] bg-[#fffbeb]' : 'border-[#ede9fe] bg-[#f5f3ff]' }}">
            <div class="flex items-center gap-4">
                <div class="flex size-12 items-center justify-center rounded-xl {{ $activeType === 'promo' ? 'bg-[#fef3c7] text-[#d97706]' : 'bg-[#ede9fe] text-[#7c3aed]' }}">
                    @if ($activeType === 'promo')
                ...
                <div>
                    <h3 class="text-base font-bold text-gray-800">
                        {{ $activeType === 'promo' ? 'Buat Promo Indonesia Luxe' : 'Buat Gift Card Indonesia Luxe' }}
                    </h3>
                    <p class="text-xs text-gray-500">
                        {{ $activeType === 'promo' ? 'Promo internal langsung aktif setelah disimpan.' : 'Gift card internal langsung aktif untuk traveler.' }}
                    </p>
                </div>
            </div>
            <button wire:click="openCreateForm" class="inline-flex items-center justify-center gap-2 rounded-xl px-5 py-2.5 text-xs font-bold text-white transition-all {{ $activeType === 'promo' ? 'bg-[#d97706] hover:bg-[#b45309]' : 'bg-[#7c3aed] hover:bg-[#6d28d9]' }}">
                ...
                {{ $activeType === 'promo' ? 'Buat Promo' : 'Buat Gift Card' }}
            </button>
        </div>
    @endif

    @if ($isCreating)
        <div class="overflow-hidden rounded-3xl border shadow-sm {{ $activeType === 'promo' ? 'border-[#ffecd5] bg-[#fffcf0]' : 'border-[#f3e8ff] bg-[#faf5ff]' }}">
            <div class="flex flex-col justify-between gap-4 border-b p-6 md:flex-row md:items-start {{ $activeType === 'promo' ? 'border-[#ffedd5]' : 'border-[#f3e8ff]' }}">
                <div>
                    <h3 class="text-base font-bold text-gray-800">
                        {{ $activeType === 'promo' ? 'Promo Internal Indonesia Luxe' : 'Gift Card Internal Indonesia Luxe' }}
                    </h3>
                    <p class="text-[11px] italic text-gray-500">
                        {{ $activeType === 'promo' ? 'Gunakan kontrak data yang sama dengan form vendor.' : 'Gift card akan disimpan sebagai item internal milik akun admin.' }}
                    </p>
                </div>
                ...
                @if ($activeType === 'promo')
                    <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                        <div class="space-y-1.5">
                            ...
                            @error('code') <span class="text-[10px] font-medium text-red-500">{{ $message }}</span> @enderror
                        </div>
                        <div class="space-y-1.5">
                            ...
                            @error('discount_type') <span class="text-[10px] font-medium text-red-500">{{ $message }}</span> @enderror
                        </div>
                    </div>
                    ...
                    <div class="space-y-1.5">
                        ...
                        @error('description') <span class="text-[10px] font-medium text-red-500">{{ $message }}</span> @enderror
                    </div>

                    <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                        <div class="space-y-1.5">
                            <label class="text-[9px] font-bold uppercase tracking-widest text-gray-400">NILAI DISKON *</label>
                            <input wire:model.live="discount_value" type="number" step="0.01" placeholder="{{ $discount_type === 'percent' ? '20' : '500000' }}" class="w-full rounded-xl border border-gray-100 bg-white px-4 py-2 text-xs outline-none focus:ring-2 focus:ring-[#f59e0b]/20" />
                            @error('discount_value') <span class="text-[10px] font-medium text-red-500">{{ $message }}</span> @enderror
                        </div>
                        <div class="space-y-1.5">
                            ...
                            @error('min_purchase') <span class="text-[10px] font-medium text-red-500">{{ $message }}</span> @enderror
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                        <div class="space-y-1.5">
                            ...
                            @error('category_restriction') <span class="text-[10px] font-medium text-red-500">{{ $message }}</span> @enderror
                        </div>
                        <div class="space-y-1.5">
                            ...
                            @error('group') <span class="text-[10px] font-medium text-red-500">{{ $message }}</span> @enderror
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                        <div class="space-y-1.5">
                            ...
                            @error('valid_from') <span class="text-[10px] font-medium text-red-500">{{ $message }}</span> @enderror
                        </div>
                        <div class="space-y-1.5">
                            ...
                            @error('valid_until') <span class="text-[10px] font-medium text-red-500">{{ $message }}</span> @enderror
                        </div>
                    </div>
                ...
                @else
                    <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                        <div class="space-y-1.5">
                            ...
                            @error('gift_code') <span class="text-[10px] font-medium text-red-500">{{ $message }}</span> @enderror
                        </div>
                        <div class="space-y-1.5">
                            ...
                            @error('gift_value') <span class="text-[10px] font-medium text-red-500">{{ $message }}</span> @enderror
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                        <div class="space-y-1.5">
                            ...
                            @error('max_usages') <span class="text-[10px] font-medium text-red-500">{{ $message }}</span> @enderror
                        </div>
                        <div class="space-y-1.5">
                            ...
                            @error('expires_at') <span class="text-[10px] font-medium text-red-500">{{ $message }}</span> @enderror
                        </div>
                    </div>
    ...
    <div class="grid grid-cols-1 gap-4 sm:gap-6 sm:grid-cols-3">
        <button wire:click="setStatus('pending_approval')" class="relative overflow-hidden rounded-3xl border bg-white p-6 text-center transition-all hover:shadow-md {{ $statusFilter === 'pending_approval' ? 'border-[#f59e0b] ring-1 ring-[#f59e0b]' : 'border-gray-100 shadow-sm' }}">
            <div class="absolute left-0 top-0 h-1 w-full bg-[#fef3c7]"></div>
            <div class="mb-1 text-[28px] font-bold text-gray-800">{{ $pendingCount }}</div>
            <div class="text-[10px] font-bold uppercase tracking-widest text-gray-400">Pending</div>
        </button>
        <button wire:click="setStatus('active')" class="relative overflow-hidden rounded-3xl border bg-white p-6 text-center transition-all hover:shadow-md {{ $statusFilter === 'active' ? 'border-[#10b981] ring-1 ring-[#10b981]' : 'border-gray-100 shadow-sm' }}">
            <div class="absolute left-0 top-0 h-1 w-full bg-[#e8fff3]"></div>
            <div class="mb-1 text-[28px] font-bold text-gray-800">{{ $approvedCount }}</div>
            <div class="text-[10px] font-bold uppercase tracking-widest text-gray-400">Approved</div>
        </button>
        <button wire:click="setStatus('rejected')" class="relative overflow-hidden rounded-3xl border bg-white p-6 text-center transition-all hover:shadow-md {{ $statusFilter === 'rejected' ? 'border-[#ef4444] ring-1 ring-[#ef4444]' : 'border-gray-100 shadow-sm' }}">
            <div class="absolute left-0 top-0 h-1 w-full bg-[#fee2e2]"></div>
            <div class="mb-1 text-[28px] font-bold text-gray-800">{{ $rejectedCount }}</div>
    ...
    <div class="flex items-center justify-between rounded-3xl border border-gray-100 bg-white p-4 shadow-sm">
        <div class="w-full max-w-md">
            <x-admin.search-input model="search" :value="$search" :debounce="500" placeholder="Cari kode, deskripsi, grup, atau nama vendor..." />
        </div>
    </div>
    ...
    <div class="space-y-4">
        @forelse ($items as $item)
            ...
            <div wire:key="{{ $activeType }}-{{ $item->id }}" class="overflow-hidden rounded-3xl border border-gray-100 bg-white shadow-sm transition-all hover:border-[#f59e0b]/30">
                <div class="flex flex-col gap-4 p-4 sm:p-6 sm:flex-row sm:items-center sm:justify-between">
                    <div class="flex items-start gap-3 sm:gap-5">
                        ...
                        <div class="space-y-1">
                            <div class="text-sm font-bold text-gray-900">{{ $item->code }}</div>
                            <div class="text-[11px] sm:text-xs text-gray-500">
                                {{ $issuerName }} •
                                @if ($activeType === 'promo')
                                    {{ $item->discount_type === \App\Enums\PromoDiscountType::Percent ? rtrim(rtrim(number_format((float) $item->discount_value, 2, ',', '.'), '0'), ','). '%' : 'Rp '.number_format((float) $item->discount_value, 0, ',', '.') }}
                                    @if ($item->category_restriction)
                                        • {{ $item->category_restriction }}
                                    @endif
                                @else
                                    Nilai Rp {{ number_format((float) $item->value, 0, ',', '.') }}
                                @endif
                                <span class="hidden sm:inline"> • {{ $item->created_at->format('j M Y') }}</span>
                            </div>
                            @if ($activeType === 'promo')
                                <p class="text-xs sm:text-sm text-gray-700">{{ $item->description }}</p>
                            @endif
                        </div>
                    </div>

                    <div class="flex items-center justify-between sm:justify-end gap-3 border-t sm:border-t-0 pt-3 sm:pt-0">
                        <x-admin.status-badge :status="match($item->status->value) {
                        ...
                        <div class="flex items-center gap-1 sm:gap-2">
                            @if ($isPending)
                                <button wire:click="approve({{ $item->id }})" class="rounded-xl p-2 text-[#10b981] transition-all hover:bg-[#e8fff3]" title="Approve">
                                ...
                                <button wire:click="confirmReject({{ $item->id }})" class="rounded-xl p-2 text-[#ef4444] transition-all hover:bg-[#fee2e2]" title="Reject">
                            ...
                            <button wire:click="toggleSelection({{ $item->id }})" class="rounded-xl p-2 text-gray-400 transition-all hover:bg-gray-50" title="Detail">
                                <svg class="size-5 transition-transform {{ $isSelected ? 'rotate-90' : '' }}" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" /></svg>
                            </button>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="border-t border-gray-100 bg-gray-50/70 px-6 py-5">
                        <div class="grid grid-cols-1 gap-4 text-sm text-gray-600 md:grid-cols-2 xl:grid-cols-4">
                            <div>
                                ...
                                <div class="mt-1 font-medium text-gray-800">{{ $issuerName }}</div>
                            </div>
                            @if ($activeType === 'promo')
                                <div>
                                    <div class="text-[10px] font-bold uppercase tracking-widest text-gray-400">Grup Promo</div>
                                    <div class="mt-1 font-medium text-gray-800">{{ $item->group ?: 'Indonesia Luxe' }}</div>
                                </div>
                                <div>
                                    ...
                                    <div class="mt-1 font-medium text-gray-800">
                                        {{ $item->valid_from?->format('d M Y') ?? 'Tidak dibatasi' }}
                                        @if ($item->valid_until)
                                ...
                                <div>
                                    ...
                                    <div class="mt-1 font-medium text-gray-800">Rp {{ number_format((float) $item->min_purchase, 0, ',', '.') }}</div>
                                </div>
                            @else
                                <div>
                                    <div class="text-[10px] font-bold uppercase tracking-widest text-gray-400">Kadaluarsa</div>
                                    <div class="mt-1 font-medium text-gray-800">{{ $item->expires_at?->format('d M Y') ?? 'Tanpa batas' }}</div>
                                </div>
                                <div>
                                    <div class="text-[10px] font-bold uppercase tracking-widest text-gray-400">Penggunaan</div>
                                    <div class="mt-1 font-medium text-gray-800">{{ $item->used_count }} dari {{ $item->max_usages }}</div>
                                </div>
                                <div>
                                    <div class="text-[10px] font-bold uppercase tracking-widest text-gray-400">Aktif</div>
                                    <div class="mt-1 font-medium text-gray-800">{{ $item->is_active ? 'Ya' : 'Tidak' }}</div>
                                </div>
                            @endif
                        </div>

                        @if ($item->status === \App\Enums\PromoStatus::Rejected && $item->rejected_reason)
                            <div class="mt-4 rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
                                <span class="font-semibold">Alasan penolakan:</span> {{ $item->rejected_reason }}
                            </div>
                        @endif

                        @if ($showRejectForm && $selectedItemId === $item->id)
                            <div class="mt-4 rounded-2xl border border-red-200 bg-white p-4">
                                ...
                                @error('rejectReason') <span class="mt-2 block text-[10px] font-medium text-red-500">{{ $message }}</span> @enderror
            ...
            <div class="rounded-3xl border border-gray-100 bg-white py-24 text-center shadow-sm">
                <div class="mx-auto mb-4 flex size-20 items-center justify-center rounded-full bg-gray-50">
                    <svg class="size-10 text-gray-300" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="{{ $activeType === 'promo' ? 'M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z' : 'M12 8v13m0-13V6a2 2 0 112 2h-2zm0 0V5.5A2.5 2.5 0 109.5 8H12zm-7 4h14M5 12a2 2 0 110-4h14a2 2 0 110 4M5 12v7a2 2 0 002 2h10a2 2 0 002-2v-7' }}" />
                    </svg>
                </div>
                <h3 class="text-sm font-bold text-gray-800">Belum ada data {{ $activeType === 'promo' ? 'promo' : 'gift card' }} untuk filter ini</h3>
            </div>
        @endforelse
    </div>
