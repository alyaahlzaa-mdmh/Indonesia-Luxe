<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div class="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm flex items-center gap-3">
            ...
            <div>
                ...
                <div class="flex items-baseline gap-2">
                    <span class="text-sm font-bold text-gray-800">{{ $report['total_transactions'] }}</span>
                    <span class="text-[9px] text-gray-400">{{ $report['month'] }}</span>
                </div>
            </div>
        </div>
        <div class="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm flex items-center gap-3">
            ...
            <div>
                ...
                <div class="flex items-baseline gap-2">
                    <span class="text-sm font-bold text-gray-800">Rp {{ number_format($report['total_revenue'], 0, ',', '.') }}</span>
        ...
        <div class="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm flex items-center gap-3">
            ...
            <div>
                ...
                <div class="flex items-baseline gap-2">
                    <span class="text-sm font-bold text-gray-800">{{ $report['confirmed_count'] }}</span>
        ...
        <div class="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm flex items-center gap-3">
            ...
            <div>
                ...
                <div class="flex items-baseline gap-2">
                    <span class="text-sm font-bold text-gray-800">{{ $report['pending_count'] }}</span>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 bg-white rounded-3xl border border-gray-100 shadow-sm p-8">
            <div class="flex justify-between items-start mb-8">
                <div>
                    <h3 class="font-bold text-gray-800 text-base">Tren Harian {{ $report['month_name'] }}</h3>
            ...
            <div class="grid grid-cols-3 gap-4 mb-8">
                <div class="bg-[#fffbeb] p-4 rounded-2xl border border-amber-50">
                    ...
                    <p class="text-lg font-black text-amber-900 leading-none">Rp {{ number_format($report['total_revenue'], 0, ',', '.') }}</p>
                </div>
                <div class="bg-[#f0fdf4] p-4 rounded-2xl border border-emerald-50">
                    ...
                    <p class="text-lg font-black text-emerald-900 leading-none">{{ $report['total_transactions'] }} order</p>
                </div>
                <div class="bg-[#eff6ff] p-4 rounded-2xl border border-blue-50">
                    ...
                    <p class="text-lg font-black text-blue-900 leading-none">{{ $report['active_days'] }} hari</p>
                </div>
            </div>
        ...
        <div class="bg-white rounded-3xl border border-gray-100 shadow-sm p-8 flex flex-col">
            ...
            <div class="relative size-48 mx-auto mb-12">
                ...
                <div class="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                    <span class="text-3xl font-black text-gray-800">{{ $report['status_distribution']['approved']['percentage'] }}%</span>
                    <span class="text-[10px] text-gray-400 font-bold uppercase tracking-widest">Success Rate</span>
                </div>
            </div>

            <div class="space-y-4">
                @foreach(['approved' => ['label' => 'Approved', 'color' => '#10b981'], 'pending' => ['label' => 'Pending', 'color' => '#f59e0b'], 'rejected' => ['label' => 'Ditolak', 'color' => '#ef4444']] as $key => $meta)
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-3">
                            <span class="size-2.5 rounded-full" style="background-color: {{ $meta['color'] }}"></span>
                            <span class="text-xs font-bold text-gray-500 uppercase tracking-widest">{{ $meta['label'] }}</span>
                        </div>
                        <div class="flex items-center gap-3">
                            <div class="w-32 bg-gray-50 h-1 rounded-full overflow-hidden">
                                <div class="h-full rounded-full" style="background-color: {{ $meta['color'] }}; width: {{ $report['status_distribution'][$key]['percentage'] }}%"></div>
                            </div>
                            <span class="text-[11px] font-black text-gray-800 w-12 text-right">{{ $report['status_distribution'][$key]['count'] }} ({{ $report['status_distribution'][$key]['percentage'] }}%)</span>
                        </div>
                    </div>
                @endforeach
            </div>
    ...
    <div class="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
        ...
        <div class="p-8 grid grid-cols-2 md:grid-cols-6 gap-8">
            <div class="space-y-1">
                ...
                <p class="text-xl font-bold text-gray-800">{{ $report['total_users'] }}</p>
            </div>
            <div class="space-y-1">
                <p class="text-[9px] text-gray-400 font-bold uppercase tracking-widest">Total Vendor</p>
                <p class="text-xl font-bold text-gray-800">{{ $report['total_vendors'] }}</p>
            </div>
            <div class="space-y-1">
                <p class="text-[9px] text-gray-400 font-bold uppercase tracking-widest">Paket Tour</p>
                <p class="text-xl font-bold text-gray-800">{{ $report['total_tours'] }}</p>
            </div>
            <div class="space-y-1">
                <p class="text-[9px] text-gray-400 font-bold uppercase tracking-widest">Tour Approved</p>
                <p class="text-xl font-bold text-gray-800">{{ $report['approved_tours'] }}</p>
            </div>
            <div class="space-y-1">
                <p class="text-[9px] text-gray-400 font-bold uppercase tracking-widest">Total Revenue</p>
                <p class="text-xl font-bold text-[#cca462]">Rp {{ number_format($report['global_total_revenue'], 0, ',', '.') }}</p>
            </div>
            <div class="space-y-1">
                <p class="text-[9px] text-gray-400 font-bold uppercase tracking-widest">Total Orders</p>
                <p class="text-xl font-bold text-gray-800">{{ $report['global_total_orders'] }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm flex items-center gap-4">
        <div class="flex-1 max-w-md">
            <x-admin.search-input model="search" :value="$search" :debounce="500" placeholder="Cari nama customer atau paket tour..." />
        </div>
    </div>
    ...
    <div class="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
        <div class="px-8 py-6 border-b border-gray-50 flex justify-between items-center bg-gray-50/10">
            <h3 class="font-bold text-gray-800 text-sm italic uppercase tracking-wider">Detail Transaksi — {{ $report['month'] }}</h3>
            <span class="text-[11px] text-gray-400 font-medium">{{ $orders->total() }} transaksi</span>
        </div>
            ...
            <table class="w-full text-sm text-left">
                ...
                <tbody class="divide-y divide-gray-50">
                    @forelse($orders as $order)
                        <tr wire:key="monthly-report-order-{{ $order->id }}" class="hover:bg-gray-50/50 transition-colors">
                            <td class="px-8 py-6 text-gray-500 whitespace-nowrap">{{ $order->created_at->format('j/n/Y') }}</td>
                            <td class="px-8 py-6 font-bold text-gray-800">{{ $order->user->name }}</td>
                            <td class="px-8 py-6 text-gray-600 max-w-xs truncate">
                                {{ $order->items->first()->package_title ?? '-' }}
                                @if($order->items->count() > 1)
                                    <span class="text-[10px] bg-gray-100 px-1 rounded ml-1">+{{ $order->items->count() - 1 }}</span>
                                @endif
                            </td>
                            <td class="px-8 py-6 font-bold text-[#cca462]">Rp {{ number_format($order->total_amount, 0, ',', '.') }}</td>
                            <td class="px-8 py-6">
                                <span class="px-3 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider {{ $order->status->colorAdmin() }}">
                                    {{ $order->status->label() }}
                                </span>
                            </td>
                        </tr>
    ...
    <script id="monthly-report-chart-data" type="application/json">@json($report['chart_payload'])</script>
