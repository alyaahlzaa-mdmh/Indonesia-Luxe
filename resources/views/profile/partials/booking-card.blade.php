<div class="px-6 py-5 border-b border-gray-50 last:border-0 hover:bg-gray-50/50 transition">
  <div class="flex items-start gap-4">
    <div class="w-20 h-20 rounded-xl overflow-hidden shrink-0 border border-gray-100">
      @if($booking->tourPackage->cover_image_path)
      <img src="{{ Storage::url($booking->tourPackage->cover_image_path) }}" class="w-full h-full object-cover">
      @else
    ...
    <div class="flex-1 min-w-0">
      <div class="flex items-start justify-between mb-1">
        <h3 class="text-sm font-bold text-gray-900 truncate pr-4">{{ $booking->tourPackage->title }}</h3>
        <span class="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full {{ $booking->status->color() }}">
      ...
      <div class="flex flex-col gap-1">
        <div class="flex items-center gap-1.5 text-xs text-gray-500">
          ...
          <span>{{ $booking->orderItem->departure_date->translatedFormat('d M Y') }}</span>
        </div>
        <div class="flex items-center gap-1.5 text-xs text-gray-500">
          ...
          <span>{{ $booking->orderItem->quantity }} {{ __('bookings.participants') }}</span>
        </div>
      </div>

      <div class="mt-3 flex items-center justify-between">
        <p class="text-sm font-bold text-amber-600">
          Rp {{ number_format($booking->orderItem->line_total, 0, ',', '.') }}
        </p>
        @if($booking->status === \App\Enums\BookingStatus::Confirmed && !$booking->review)
        <button class="text-[11px] font-bold text-amber-500 hover:text-amber-600 transition underline underline-offset-4 cursor-pointer">{{ __('bookings.give_review') }}</button>
        ...
        <div class="flex items-center gap-0.5">
          ...
          <span class="text-[10px] font-bold text-gray-700">{{ $booking->review->rating }}</span>
        </div>
        @endif
      </div>
    </div>
